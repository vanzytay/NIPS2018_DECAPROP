...
def convert_paragraph_v2(para):
    ...
    # context_tokens = context_tokens[:hard_limit]
...
train_words, train_data, train_feats = load_set('./corpus/Quasar/train.json')
dev_words, dev_data, dev_feats = load_set('./corpus/Quasar/dev.json')
test_words, test_data, test_feats = load_set('./corpus/Quasar/test.json')
...
dictToFile(env,'./datasets/Quasar/env.gz'.format(mode))
dictToFile(feature_env,'./datasets/Quasar/feats.gz'.format(mode))
